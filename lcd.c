 ...
 * port to Turnigy TGY-i6: stinkydiver73@gmail.com
 */
...
#define LCD_E			(30)	// E (marked R0)
#define LCD_RW			(31)	// R#W (marked RW)
#define LCD_A0			(13)	// A0  (marked RS)
#define LCD_RES			(7)	// Reset
#define LCD_CS			(4)	// Chip Select 1
...
static uint32_t porttemp, datatemp;
...
static void lcd_send_command(uint8_t cmd) {
	palClearPad(GPIOA,LCD_A0);
	palClearPad(GPIOE,LCD_RW);

	palClearPad(GPIOA,LCD_CS);

	chSysLock();
	porttemp=(0xffc3ffff & palReadLatch(GPIOE));
	datatemp=((uint32_t)(cmd<<28)>>10);
	palWritePort(GPIOE, (porttemp | datatemp));
	chSysUnlock();
	chSysLock();
	porttemp=(0xfffff0ff & palReadLatch(GPIOC));
	datatemp=((cmd>>4)<<8);
	palWritePort(GPIOC, (porttemp | datatemp));
	chSysUnlock();

	palSetPad(GPIOE,LCD_E);
	//chThdSleepMicroseconds(5); //delay_us(5)
	palClearPad(GPIOE,LCD_E);

	palSetPad(GPIOA,LCD_CS);
}
...
void lcd_init(void) {
	...
	palSetPadMode(GPIOA,  4, PAL_MODE_OUTPUT_PUSHPULL); //CS
	palSetPadMode(GPIOA, 13, PAL_MODE_OUTPUT_PUSHPULL); //A0
	palSetPadMode(GPIOD,  7, PAL_MODE_OUTPUT_PUSHPULL); //RST
	palSetPadMode(GPIOE, 30, PAL_MODE_OUTPUT_PUSHPULL); //E
	palSetPadMode(GPIOE, 31, PAL_MODE_OUTPUT_PUSHPULL); //R#W

	palSetPadMode(GPIOE, 18, PAL_MODE_OUTPUT_PUSHPULL); //D0 
	palSetPadMode(GPIOE, 19, PAL_MODE_OUTPUT_PUSHPULL); //D1
	palSetPadMode(GPIOE, 20, PAL_MODE_OUTPUT_PUSHPULL); //D2
	palSetPadMode(GPIOE, 21, PAL_MODE_OUTPUT_PUSHPULL); //D3
	palSetPadMode(GPIOC,  8, PAL_MODE_OUTPUT_PUSHPULL); //D4
	palSetPadMode(GPIOC,  9, PAL_MODE_OUTPUT_PUSHPULL); //D5
	palSetPadMode(GPIOC, 10, PAL_MODE_OUTPUT_PUSHPULL); //D6
	palSetPadMode(GPIOC, 11, PAL_MODE_OUTPUT_PUSHPULL); //D7
	
	palClearPad(GPIOD, LCD_E);
	palClearPad(GPIOE,LCD_RW);
	palSetPad(GPIOA,LCD_A0);
	palSetPad(GPIOA,LCD_CS);

	// Reset LCD
	palClearPad(GPIOD, LCD_RES);
	chThdSleepMicroseconds(5); //delay_us(5)
	palSetPad(GPIOD, LCD_RES);
	...
	lcd_send_command(KS0713_ADC_SELECT | 0x00); // Select SEG output direction reversed (ADC = 1)
	...
	lcd_send_command(KS0713_SHL_SELECT | 0x00); // Select COM output direction normal (SHL = 0)
...
void lcd_backlight(bool state) {
	if (state)
		palSetPad(GPIOB, LCD_BACKLIGHT);
	else
		palClearPad(GPIOB, LCD_BACKLIGHT);
}
...
void lcd_update(void) {
	...
	for (row = 0; row < LCD_HEIGHT / 8; ++row) {
		...
		lcd_send_command(KS0713_SET_COL_ADDR_LSB | 0x00); // low col
